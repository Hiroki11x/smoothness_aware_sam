...
class Config:
    ...
    calc_gradnorm: bool
    calc_hessian: bool
    pyhessian_interval: int
    pyhessian_batch_size_per_gpu: int
    pyhessian_sample_size: int
    pyhessian_top_n: int
    # ------------------------------- MODEL -------------------------------------
    model: str
    dropout: float
...
def get_parser() -> argparse.ArgumentParser:
    ...
    # It was not used in the original code
    # argparser.add_argument(
    #     "--log_interval", default=20, type=int, help="Log every n steps"
    # )
    argparser.add_argument(
        "--no_tqdm", type=str2bool, default=True
    )
    # ------------------------------- WANDB -------------------------------------
    argparser.add_argument("--wandb_entity", type=str, default="")
    argparser.add_argument("--wandb_project_name", type=str, default="wasam")
    argparser.add_argument("--wandb_exp_id", type=int, default=999999)
    argparser.add_argument('--wandb_offline', type=str2bool, default=False)

    # ------------------------------- CALC HESSIAN -------------------------------------
    argparser.add_argument('--calc_gradnorm', type=str2bool, default=False)
    argparser.add_argument('--calc_hessian', type=str2bool, default=False)
    argparser.add_argument('--pyhessian_interval', type=int, default=25)
    argparser.add_argument('--pyhessian_batch_size_per_gpu', type=int, default=64)
    argparser.add_argument('--pyhessian_sample_size', type=int, default=1024)
    argparser.add_argument('--pyhessian_top_n', type=int, default=1)
    ...
    argparser.add_argument("--img_size", default=32, type=int) # For ViT should be set as 224
    argparser.add_argument("--pretrained_path", default='./', type=str)
    # ------------------------------- REGULARIZATION -------------------------------------
    ...
    argparser.add_argument("--grad_clip", default=0.0, type=float, help="Gradient clipping value")
    # ------------------------------- EXPERIMENT -------------------------------------
    argparser.add_argument(
        "--num_workers",
        default=8,
    ...
    argparser.add_argument(
        ...
        choices=[
            ...
            "adamw",
            "sam",
            # "swa", # this is not supported  -> use sgd + is_swa = True
            # "asam", # this is not supported -> use sam + is_swa = True
            "sgd",
            "sasgd", # Smoothness Aware SGD
            "sasam", # Smoothness Aware SAM
        ],
        help="optimizer name / model averaging is controlled by is_swa",
    )

    # this is no longer used / base optimizer should be determined by optimizer
    # argparser.add_argument(
    ...
    argparser.add_argument("--is_warmup", type=str2bool, default=False)
    argparser.add_argument("--warmup_steps", type=int, default=500)
    ...
    argparser.add_argument("--sam_rho_fixed", type=str2bool, default=False)
    # ------------------------------- SA: Smoothness Aware -------------------------------------
    argparser.add_argument("--amplifier", type=float, default=0.02)
    argparser.add_argument("--is_sa", type=str2bool, default=False)
    # ------------------------------- SWA -------------------------------------
